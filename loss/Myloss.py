...
#衡量两个输入张量x和y之间的颜色相似度
    ...
    def forward(self, x, y):
        ...
        product_separte_color = (x*y).mean(1,keepdim=True)#x*y表示输入张量x和y的乘积，然后对第二个维度求均值并保留维度；即得到每个像素点颜色通道的乘积的均值
        x_abs = (x**2).mean(1,keepdim=True)**0.5#x_abs=输入张量x中每个像素点的平方的均值再开方；即每个像素点颜色通道的绝对值的平均值
        y_abs = (y**2).mean(1,keepdim=True)**0.5#y_abs=输入张量y中每个像素点的平方的均值再开方；即每个像素点颜色通道的绝对值的平均值
        #(1-product_separte_color/(x_abs*y_abs+0.00001)).mean()表示计算颜色相似度的误差；
        #torch.mean(torch.acos(product_separte_color/(x_abs*y_abs+0.00001))表示计算颜色相似度的角度误差；
...
class L_spa(nn.Module):
    ...
    def forward(self, org , enhance ):
        b,c,h,w = org.shape#分别代表batch size、通道数、高度和宽度。
        # 计算通道维度上的均值
        ...
        # weight_diff=max(1 + 10000 * min(org_pool - 0.3, 0), 0.5)
        #主要用于衡量图像增强对失真的影响程度，如果图像增强的失真较小，则 weight_diff 趋近于 1，否则趋近于 0.5。
        weight_diff =torch.max(torch.FloatTensor([1]).cuda() + 10000*torch.min(org_pool - torch.FloatTensor([0.3]).cuda(),torch.FloatTensor([0]).cuda()),torch.FloatTensor([0.5]).cuda())
        #E_1=sign(enhance_pool - 0.5) * (enhance_pool - org_pool)
        #表示增强后的图像与原始图像的差异，取值为正表示增强效果好，取值为负则表示增强效果差。
...
class L_exp(nn.Module):
    ...
    def forward(self, x, mean_val ):
        ...
        b,c,h,w = x.shape
        x = torch.max(x,1,keepdim=True)[0]#第一个维度获取最大值，形状为（b,1,h,w）
        mean = self.pool(x)#池化操作，形状为（b,1,1,1）
        # d=mean 与 mean_val 之间的平均平方误差
...
class L_TV(nn.Module):
    ...
    def forward(self,x):
        ...
        h_tv = torch.pow((x[:,:,1:,:]-x[:,:,:h_x-1,:]),2).sum()#计算垂直方向的像素间差异的平方和。
        w_tv = torch.pow((x[:,:,:,1:]-x[:,:,:,:w_x-1]),2).sum()#计算水平方向的像素间差异的平方和。
